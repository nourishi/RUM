...
class Trainer:
    ...
    EPSILON = 1e-8
    BEST_TRAIN_LOSS_KEY = "Best/train_main_loss"
    ...
    def run_train(self):
        ...
        while self.epoch < self.max_epochs:
            ...
            self._save_best_train_loss_checkpoint(outs)
    ...
    def _save_best_train_loss_checkpoint(self, outs: Dict[str, float]) -> None:
        """
        Save an extra checkpoint whenever the main training objective improves.
        Auxiliary LFM control losses are excluded from this selection metric.
        """
        train_loss_key = "Losses/train_all_loss"
        if train_loss_key not in outs:
            return

        current_loss = float(outs[train_loss_key])
        current_loss -= float(outs.get("Losses/train_loss_lfm_uncertainty", 0.0))
        current_loss -= float(outs.get("Losses/train_loss_lfm_policy", 0.0))

        best_loss = self.best_meter_values.get(self.BEST_TRAIN_LOSS_KEY, None)
        if best_loss is None or current_loss < best_loss:
            self.best_meter_values[self.BEST_TRAIN_LOSS_KEY] = current_loss
            logging.info(
                f"New best train main loss: {current_loss:.6f} at epoch {int(self.epoch)}"
            )
            self.save_checkpoint(
                self.epoch + 1,
                checkpoint_names=["checkpoint_best_train_main_loss", "checkpoint_best_train_loss"],
            )
