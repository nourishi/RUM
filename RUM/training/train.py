...
def compose_config(config_name, hydra_overrides=None):
    overrides = hydra_overrides or []

    # Support direct config file paths, e.g., /repo/sam2/configs/...yaml
    if os.path.isfile(config_name):
        config_name = os.path.abspath(config_name)
        config_dir = os.path.dirname(config_name)
        cfg_name = os.path.basename(config_name)
    ...
    try:
        ...
        # Fallback for source checkout where local sam2 package is not on PYTHONPATH.
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        local_config_dir = os.path.join(repo_root, "sam2")
        if not os.path.isdir(local_config_dir):
            raise
        if GlobalHydra.instance().is_initialized():
            GlobalHydra.instance().clear()
        initialize_config_dir(config_dir=local_config_dir, version_base="1.2")
        return compose(config_name=config_name, overrides=overrides)


def main(args, hydra_overrides=None) -> None:
    cfg = compose_config(args.config, hydra_overrides=hydra_overrides)
    if cfg.launcher.experiment_log_dir is None:
        ...
        cfg.launcher.experiment_log_dir = os.path.join(
            os.getcwd(), "sam2_logs", cfg_log_name
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="path to config file (e.g. sam2/configs/rum_training/rum_training.yaml)",
    )
    parser.add_argument(
        "--use-cluster",
        type=int,
    ...
    register_omegaconf_resolvers()
    main(args, hydra_overrides=hydra_overrides)
