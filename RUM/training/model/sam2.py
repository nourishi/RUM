...
class SAM2Train(SAM2Base):
    def __init__(
        ...
        freeze_base_model_except_language_free=False,
        **kwargs,
    ):
        ...
        if freeze_base_model_except_language_free:
            self._freeze_all_except_language_free_modules()

    def _freeze_all_except_language_free_modules(self):
        trainable_prefixes = (
            "usm_scheduler",
            "visual_prototype_memory",
            "motion_relocalizer",
        )
    ...
    def track_step(
        ...
        if current_out["multistep_pred_multimasks"][-1].shape[1] == 1:
            current_out["multistep_pred_multimasks"][-1] = low_res_masks
        if current_out["multistep_pred_multimasks_high_res"][-1].shape[1] == 1:
            current_out["multistep_pred_multimasks_high_res"][-1] = high_res_masks
        current_out["multistep_object_score_logits"][-1] = object_score_logits
        prev_low_res_steps = current_out["multistep_pred_masks"]
        if prev_low_res_steps.shape[1] == 1:
            current_out["multistep_pred_masks"] = low_res_masks
        else:
            current_out["multistep_pred_masks"] = torch.cat(
                [prev_low_res_steps[:, :-1], low_res_masks], dim=1
            )

        prev_high_res_steps = current_out["multistep_pred_masks_high_res"]
        if prev_high_res_steps.shape[1] == 1:
            current_out["multistep_pred_masks_high_res"] = high_res_masks
        else:
            current_out["multistep_pred_masks_high_res"] = torch.cat(
                [prev_high_res_steps[:, :-1], high_res_masks], dim=1
            )
        ...
        if self.use_language_free_vos:
            ...
            current_out["lfm_write_prob"] = lfm_outputs["write_prob"]
            current_out["lfm_freeze_prob"] = lfm_outputs["freeze_prob"]
            current_out["lfm_relocalize_prob"] = lfm_outputs["relocalize_prob"]
