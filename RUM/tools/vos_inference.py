...
def setup_logging(output_mask_dir):
    os.makedirs(output_mask_dir, exist_ok=True)
    log_path = os.path.join(output_mask_dir, "vos_inference.log")
...
def main():
    ...
    parser.add_argument(
        "--rum_cfg",
        type=str,
        default="configs/rum/rum.yaml",
        help="RUM model configuration file",
    )
    parser.add_argument(
        "--rum_checkpoint",
        type=str,
        required=True,
        help="path to the RUM model checkpoint",
    )
    parser.add_argument(
        "--base_video_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        "(default without this flag: just the first PNG file as input to the model; "
    ...
    parser.add_argument(
        ...
        "(disabled by default for evaluation)",
    )
    parser.add_argument(
    ...
    setup_logging(args.output_mask_dir)
    ...
    predictor = build_sam2_video_predictor(
        config_file=args.rum_cfg,
        ckpt_path=args.rum_checkpoint,
    ...
    if args.use_all_masks:
        print("using all available masks in input_mask_dir as input to the model")
    else:
        print(
            "using only the first frame's mask in input_mask_dir as input to the model"
        )
